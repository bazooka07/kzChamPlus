...
<p>
	Le plugin chamPlus propose 5 hooks pour afficher les champs supplémentaires ou tous les champs des articles, à utiliser dans les gabarits. Pour afficher des champs supplémentaire sur votre site, utiliser le hook chamPlus dans vos gabarits (<i>template</i>) article.php ou static.php ou categorie.php comme ci-dessous. Il est conseillé de dupliquer les gabarits originaux en les renommant <i>( attention, les noms sont sensibles à la casse )</i> :
</p>
<ul>
	<li><a href="#chamPlus">chamPlus()</a></li>
...
<h2 id="chamPlus">Hook chamPlus()</h2>
<p>
...
<p>
	...
	<ul>
	 <li>Le premier paramètre obligatoire sera le nom du champ comme précédemment.</li>
	 <li>Le deuxième paramètre optionnel sera utilisé lorsque le champ n'est pas vide, avec une des valeurs suivantes:
		<ul>
	...
	<li>Le troisième paramètre optionnel sera utilisé lorsque le champ est vide. Il doit être obligatoirement de type string pour être pris en compte. C'est la chaine de format, avec les mêmes possibilités que le deuxième paramètre, qui sera utilisée lorsque le champ est vide.</li>
</ul>
...
&dollar;a = &dollar;plxShow->callHook('chamPlus', &dollar;params);
?>
</code></pre>
<p>
...
<pre><code>&lt;pre>&lt;?php print_r(eval(&dollar;plxShow->callHook('chamPlusList'))); ?>&lt;/pre></code></pre>
<a href="#top">Début de l'aide</a>
...
<p>
Si par exemple, nous ajoutons le champ supplémentaire vignette, nous pouvons afficher les articles avec ce champ pour une catégorie donnée avec le gabarit, nommé par exemple categorie-chamPlus-article, ci-dessous pour cette catégorie. Une règle CSS simple permet d'afficher sur deux colonnes, avec le champ vignette sur la première colonne :
</p>
<pre><code>&lt;?php include(dirname(__FILE__).'/header.php'); ?>
	&lt;main>
...
<pre><code>#vignettes-cat { display: flex; }</code></pre>
<p>
...
<p>
La version 5.5 de Pluxml introduit ce nouveau hook qui simplifie beaucoup l'intégration du plugin chamPlus dans Pluxml.
...
<pre><code>&lt;?php &dollar;plxShow->lastArtList('#cps_vignette');</code></pre>
...
<pre><code>&lt;img src="mon_image.jpg" height="xxxx" width="yyyy"
